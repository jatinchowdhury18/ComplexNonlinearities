...
to enhance linear models of analog ladder filters. In this writing, we strive
...
\end{equation}
%
And the pole frequencies are equal to (assuming complex poles):
\begin{equation}
...
nonlinear function $f_{NL}(x)$. As seen in \cite{Vadim}, it will often
be convenient to consider the input-dependent ``gain'' of the nonlinear
function:
%
\begin{equation}
...
\begin{split}
    ...
    = \arctan \left(\pm \frac{\sqrt{|a_1^2- 4\frac{g_2}{g_1}a_2|}}{a_1} \right)
\end{split}
...
of this pole movement will be seen later for an example filter (see \cref{pzPlots}).
...
\end{equation}
%
Again, we can replace the nonlinear elements with input-dependent
gains:
%
\begin{equation}
    \begin{split}
        g_1 =  \frac{f_{NL}(y[n-1])}{y[n-1]} \\
        g_2 =  \frac{f_{NL}(y[n-1])}{y[n-1]}
    \end{split}
        \label{eq:gs2}
\end{equation}
%
...
\begin{equation}
    \begin{split}
        y[n] = b_0 u[n] + b_1 u[n-1] + b_2 u[n-2] \\ - a_1 g_1 y[n-1] - a_2 g_2 y[n-2]
    \end{split}
...
before passing it into the filter. Again, an example of this pole movement will be
seen in a later section (see \cref{pzPlots2}).
...
\begin{figure*}[!ht]
    ...
    \caption{\label{pzPlots2}{\it Instantaneous poles for a resonant
                                lowpass filter with nonlinear at varying input levels.}}
\end{figure*}
%
...
a resonant lowpass filter using both of structures, and shown that the
